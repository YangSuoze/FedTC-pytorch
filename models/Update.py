...
class LocalUpdate(object):
    ...
    def train(self, net, body_lr, head_lr):
        ...
        for iter in range(local_eps):
            ...
            if len(batch_loss) != 0:
                logging.info(f"local epoch:{sum(batch_loss) / len(batch_loss)}")
...
class LocalUpdateFedTKF(object):
    ...
    def train(
        self, net, body_lr, head_lr
    ):
        net.train()
        body_params = [
            p for name, p in net.named_parameters() if self.args.UDL not in name
        ]
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for l_param, g_param in zip(net.parameters(), g_net.parameters()):
                    fed_prox_reg += (
                        self.args.mu / 2 * torch.norm((l_param - g_param)) ** 2
                    )
                loss += fed_prox_reg
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if w_ditto is not None:
                    ...
                    for key in w_net.keys():
                        w_net[key] = w_net[key] - args.lr * lam * (
                            w_0[key] - w_ditto[key]
                        )
